...
uint8_t palette_color_from_rgb(uint8_t r,uint8_t g,uint8_t b)
{
    if(r==0&&g==0&&b==0)return 0;

uint8_t closest_color=255;//Color that I don't think is ever used
uint32_t least_distance=255*255*3+1;//Greatest possible distance between colors
    for(int i=10;i<226;i++)
    {
    color_t color=palette_color_from_index(i);
    int32_t diff_r=r-color.red;
    int32_t diff_g=g-color.green;
    int32_t diff_b=b-color.blue;
    int32_t distance=diff_r*diff_r+diff_g*diff_g+diff_b*diff_b;
